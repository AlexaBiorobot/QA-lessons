...
def fetch_csv(ss_id: str, gid: str) -> pd.DataFrame:
    ...
    resp = api_retry(requests.get, url, timeout=20); resp.raise_for_status()
    try:
...
def fetch_values(ss_id: str, sheet_name: str) -> list[list[str]]:
    ...
    resp    = api_retry(requests.get, url, headers=headers); resp.raise_for_status()
...
def load_rating(ss_id: str) -> pd.DataFrame:
    ...
    if not rows or len(rows) < 2:
        return pd.DataFrame(columns=want)
    header, data = (rows[0], rows[1:]) if "Tutor ID" in rows[0] else (rows[1], rows[2:])
...
def build_df():
    ...
    # — Rating: переименуем, сольём и дропнем суффиксы
    rating_cols = ["Rating","Num of QA scores",
                   "Num of QA scores (last 90 days)","Average QA score",
                   "Average QA score (last 2 scores within last 90 days)",
                   "Average QA marker","Average QA marker (last 2 markers within last 90 days)"]
    r_lat = load_rating(RATING_LATAM_SS).rename(columns={c: f"{c}_lat" for c in rating_cols})
    r_brz = load_rating(RATING_BRAZIL_SS).rename(columns={c: f"{c}_brz" for c in rating_cols})
    df = df.merge(r_lat, on="Tutor ID", how="left").merge(r_brz, on="Tutor ID", how="left")
